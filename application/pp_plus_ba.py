...
class PpPlusWorker:
    ...
    def load_settings(self):
        ...
        for key in ("sms_provider", "sms_country", "flow_country", "proxy_mode", "proxy_api_url", "proxy_pool_text"):
            if key in data:
    ...
    def get_status(self):
        with self._lock:
            ...
            return {
                ...
                "sms_service_code": PAYPAL_SMS_SERVICE_CODES.get(_text(settings.get("sms_provider")), "pp"),
    ...
    def _pick_next_account(self):
        with Session(engine) as session:
            rows = session.exec(
                select(AccountModel).where(AccountModel.platform == "chatgpt").order_by(AccountModel.id.asc())
            ).all()
            if not rows:
                return None
            graphs = load_account_graphs(session, [int(m.id or 0) for m in rows if m.id])
            for model in rows:
                ...
                token = _account_ba_token(payload)
                if not token:
                    continue
                if not _is_free_plan(payload):
                    continue
                status = _text(overview.get("pp_task_status")).lower()
    ...
    def _build_sms_provider(self, settings):
        ...
        service = PAYPAL_SMS_SERVICE_CODES.get(provider_key, "pp")
    ...
    def _process_account(self, account):
        ...
        try:
            ...
            self._set_account_stage(account_id, email=email, status="error", stage="任务失败", error=err, ba_token="", extra_overview={"pp_payment_status": "failed"})
    ...
    def _run_paypal_for_account(self, account, ba_token, settings):
        ...
        self._set_account_stage(account_id, email=email, status="running", stage="正在获取手机号", ba_token=ba_token)
        activation = sms_provider.get_number(service=service_code, country=_text(settings.get("sms_country")))
...
class AutoSmsPayPalFlow(PayPalFlow):
    ...
    def _rent_new_phone(self):
        ...
        try:
            ...
            pass
        activation = self.sms_provider.get_number(service=self.sms_service, country=self.sms_country)
    ...
    def _wait_sms_code(self, timeout=60):
        ...
        if hasattr(self.sms_provider, "wait_for_code"):
            try:
                payload = self.sms_provider.wait_for_code(activation_id, timeout=timeout, poll_interval=3)
        ...
        try:
            return _text(self.sms_provider.get_code(activation_id, timeout=timeout))
        except Exception as exc:
            raise TimeoutError(str(exc) or "等待验证码超时") from exc
