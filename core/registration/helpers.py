...
def build_phone_callbacks(ctx: RegistrationContext, *, service: str | None = None):
    ...
    # Provider-specific country fields take priority when the current provider is known,
    # because country codes differ between platforms (e.g. SMSBower Indonesia=6,
    # SMSPool Indonesia=9, Philippines=12).
    _provider_country_fields: list[str] = {
        "smspool_api": ["smspool_country", "smspool_default_country"],
        "smsbower_api": ["smsbower_country", "smsbower_default_country"],
        "grizzlysms_api": ["grizzlysms_country", "grizzlysms_default_country"],
        "sms_verification_number_api": ["sms_verification_number_country", "sms_verification_number_default_country"],
        "five_sim_api": ["five_sim_country", "five_sim_default_country"],
        "nexsms_api": ["nexsms_country", "nexsms_default_country", "nexsms_country_order"],
    }.get(provider_key, [])
    country = ""
    for _f in _provider_country_fields:
        _v = str(merged.get(_f) or "").strip()
        if _v:
            country = _v
            break
    if not country:
    ...
    _provider_service_fields: list[str] = {
        "smspool_api": ["smspool_service", "smspool_default_service"],
        "smsbower_api": ["smsbower_service", "smsbower_default_service"],
        "grizzlysms_api": ["grizzlysms_service", "grizzlysms_default_service"],
        "sms_verification_number_api": ["sms_verification_number_service", "sms_verification_number_default_service"],
        "five_sim_api": ["five_sim_product"],
        "nexsms_api": ["nexsms_service", "nexsms_default_service"],
    }.get(provider_key, [])
    sms_service = ""
    for _f in _provider_service_fields:
        _v = str(merged.get(_f) or "").strip()
        if _v:
            sms_service = _v
            break
