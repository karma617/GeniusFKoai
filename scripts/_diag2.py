...
print('init: amt=' + amt + ' pmt=' + str(pmt))
...
cdata = cr.json()
print('confirm: ' + str(cr.status_code))

sa = cdata.get('submission_attempt') or {}
print('submission_attempt: ' + json.dumps(sa, ensure_ascii=False)[:800])

si = cdata.get('setup_intent')
pi = cdata.get('payment_intent')
if si: print('setup_intent: ' + json.dumps(si, ensure_ascii=False)[:400])
if pi: print('payment_intent: ' + json.dumps(pi, ensure_ascii=False)[:400])
