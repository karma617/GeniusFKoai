...
amt_init = str((init.get('elements_options') or {}).get('amount') or inv.get('amount_due') or '0')
bca = inv.get('billing_cycle_anchor','')
print('init: amt=' + amt_init + ' bca=' + str(bca)[:30] + ' pmt=' + str(init.get('payment_method_types')))

# tax_region update (US)
...
tax_data = tax_r.json() if tax_r.status_code == 200 else {}
checksum = tax_data.get('init_checksum', checksum)
inv2 = tax_data.get('invoice') or {}
amt_tax = str((tax_data.get('elements_options') or {}).get('amount') or inv2.get('amount_due') or amt_init)
bca2 = inv2.get('billing_cycle_anchor','')
print('tax: amt=' + amt_tax + ' bca=' + str(bca2)[:30])

# Determine expected_amount and expected_amount_on_bca
# If there's a billing_cycle_anchor and no prorations, expected_amount=0, expected_amount_on_bca=amount_due
has_bca = bool(str(bca2 or '').strip())
has_proration = False
if isinstance(inv2, dict):
    has_proration = bool(inv2.get('has_prorations'))
if has_bca and not has_proration:
    expected_amount = '0'
    expected_amount_on_bca = amt_tax
else:
    expected_amount = amt_tax
    expected_amount_on_bca = ''
print('expected: amt=' + expected_amount + ' bca=' + (expected_amount_on_bca or '-'))
...
# confirm with correct expected_amount
...
print('approve: ' + str(ar.status_code) + ' ' + ar.text[:150])
...
for i in range(10):
    ...
    if pr.status_code == 200:
        ...
        sa_err = psa.get('error') if isinstance(psa, dict) else None
        print('poll[' + str(i) + ']: sa=' + str(sa_state) + ' err=' + str(sa_err)[:120] if sa_err else 'poll[' + str(i) + ']: sa=' + str(sa_state))
        rurl = find_redirect(pdata)
        if rurl:
            print('BA FOUND: ' + rurl[:120])
            break
    else:
        print('poll[' + str(i) + ']: ' + str(pr.status_code))
else:
    print('NO BA after 10 polls')
