...
def try_once(r1, r2):
    res = {'r1': r1, 'r2': r2, 'ba_ok': False, 'ba': '', 'ip': '', 'amt': '', 'approve': 0, 'sa': '', 'err': ''}
    try:
        p1 = fetch_proxy(r1)
        s1 = make_session(p1)
        res['ip'] = p1.split('@')[1].split(':')[0] if '@' in p1 else ''
        ...
        if tr.status_code == 200:
            td = tr.json(); ck = td.get('init_checksum', ck); amt = str((td.get('elements_options') or {}).get('amount') or (td.get('invoice') or {}).get('amount_due') or amt0); res['amt'] = amt
        else: amt = amt0
        ...
        cd = cr.json(); sa = cd.get('submission_attempt') or {}; ss = sa.get('state','') if isinstance(sa, dict) else ''; res['sa'] = ss
        pmr_url = find_redirect(cd)
        if pmr_url:
            res['pm_redirect'] = pmr_url
            ...
            if bm: res['ba'] = bm.group(1); res['ba_ok'] = True; res['ba_url'] = fin[:200]
            else: res['err'] = 'no ba: ' + fin[:60]
            return res
        if ss != 'requires_approval': res['err'] = 'sa=' + ss; return res
        ...
        for i in range(6):
            time.sleep(2)
            pr = s2.get(f'{STRIPE_API_BASE}/payment_pages/{cs}', params=pp, headers=ph, timeout=5)
            if pr.status_code == 200:
                ...
                if pmr_url:
                    ...
                    return res
                psa = pd.get('submission_attempt') or {}; pst = psa.get('state','') if isinstance(psa, dict) else ''; res['sa'] = pst
                if pst == 'failed':
                    pe = psa.get('error',{}) if isinstance(psa, dict) else {}
                    res['err'] = 'failed:' + str(pe.get('code','') if isinstance(pe, dict) else ''); return res
        res['err'] = 'timeout sa=' + str(res['sa'])
...
# US x10 (验证概率性)
print('=== US x10 ===', flush=True)
for i in range(10):
    r = try_once('US', 'US')
    results.append(r)
    print(f'  [{i+1:2d}] ba={str(r["ba_ok"]):5s} ip={r.get("ip",""):15s} amt={str(r.get("amt","")):6s} approve={r.get("approve",0)} sa={r.get("sa",""):20s} err={r.get("err","")[:40]}', flush=True)
    if r['ba_ok']: print(f'       BA={r["ba"]}', flush=True)
    time.sleep(1)
...
with open('scripts/ba_chain_multi_ip_results.json', 'w') as f:
    json.dump(results, f, ensure_ascii=False, indent=2)

# 汇总
print('\n\n=== SUMMARY ===', flush=True)
from collections import Counter
for region in ['US', 'BR', 'JP', 'VN', 'TH']:
    subset = [r for r in results if r['r2'] == region]
    total = len(subset)
    ok = sum(1 for r in subset if r['ba_ok'])
    errs = Counter(r['err'][:30] for r in subset if not r['ba_ok'])
    print(f'{region}: {ok}/{total} success', flush=True)
    for err, cnt in errs.most_common(3):
        print(f'  {cnt}x {err}', flush=True)
