...
print('=== DE/EUR full flow ===', flush=True)
p = fetch_proxy('US')
s = make_session(p)
...
addr = {'name': 'Max Mustermann', 'email': 'test@example.com', 'country': 'DE', 'state': '', 'city': 'Berlin', 'postal_code': '10115', 'line1': 'Friedrichstrasse 1'}
pl = {'plan_name': 'chatgptplusplan', 'billing_details': {'country': 'DE', 'currency': 'EUR'}, 'entry_point': 'all_plans_pricing_modal', 'promo_campaign': {'promo_campaign_id': 'plus-1-month-free', 'is_coupon_from_query_param': False}, 'checkout_ui_mode': 'custom'}
...
tb = {'eid': str(uuid.uuid4()), 'tax_region[country]': 'DE', 'tax_region[postal_code]': '10115', 'tax_region[line1]': 'Friedrichstrasse 1', 'tax_region[city]': 'Berlin', 'key': pk, '_stripe_version': STRIPE_INIT_VERSION}
tr = s.post(f'{STRIPE_API_BASE}/payment_pages/{cs}', data=tb, headers=sh, timeout=30)
print(f'tax DE: {tr.status_code}', flush=True)
