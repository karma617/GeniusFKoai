...
def diag_once(label, r1, r2, max_poll=12):
    log = {'label': label, 'r1': r1, 'r2': r2}
    try:
        p1 = fetch_proxy(r1)
        s1 = make_session(p1)
        ...
        pmr_url_confirm = find_redirect(cd)
        log['redirect_in_confirm'] = bool(pmr_url_confirm)
        if pmr_url_confirm:
            log['pm_redirect'] = pmr_url_confirm[:80]
            rg = s2.get(pmr_url_confirm, allow_redirects=True, timeout=20, headers={'Referer': 'https://pay.openai.com/'})
            fin = str(getattr(rg, 'url', '')); bm = re.search(r'ba_token=(BA-[A-Za-z0-9]+)', fin)
            if bm: log['ba_token'] = bm.group(1); log['ba_url'] = fin[:200]; log['result'] = 'SUCCESS_CONFIRM_REDIRECT'
            else: log['result'] = 'NO_BA_AT_CONFIRM'; log['final_url'] = fin[:200]
            return log
        log['needs_approve'] = True
        ...
        try:
            approve_body = ar.json()
            log['approve_body'] = json.dumps(approve_body, ensure_ascii=False)[:300]
        except: log['approve_body'] = ar.text[:300]
        if ar.status_code != 200: log['result'] = 'APPROVE_FAILED'; return log
        ...
        for i in range(max_poll):
            ...
            try:
                ...
                poll_states.append(f'poll{i}:err:{str(e)[:40]}')
                continue
            if pr.status_code == 200:
                pd = pr.json()
                sa2 = pd.get('submission_attempt') or {}; pst = sa2.get('state','') if isinstance(sa2, dict) else ''
                pmr_url_poll = find_redirect(pd)
                poll_states.append(f'poll{i}:sa={pst}:redirect={bool(pmr_url_poll)}')
                if pmr_url_poll:
                    log['pm_redirect'] = pmr_url_poll[:80]
                    rg = s2.get(pmr_url_poll, allow_redirects=True, timeout=20, headers={'Referer': 'https://pay.openai.com/'})
                    fin = str(getattr(rg, 'url', '')); bm = re.search(r'ba_token=(BA-[A-Za-z0-9]+)', fin)
                    if bm: log['ba_token'] = bm.group(1); log['ba_url'] = fin[:200]; log['result'] = f'SUCCESS_POLL{i}'; return log
                    else: log['result'] = f'NO_BA_AT_POLL{i}'; log['final_url'] = fin[:200]; return log
                if pst == 'failed':
                    pe = sa2.get('error',{}) if isinstance(sa2, dict) else {}
                    log['result'] = f'FAILED_POLL{i}'; log['poll_error'] = json.dumps(pe, ensure_ascii=False)[:200] if isinstance(pe, dict) else str(pe)[:200]; return log
            else:
                poll_states.append(f'poll{i}:http={pr.status_code}')
        log['poll_states'] = poll_states
        log['result'] = 'TIMEOUT_NO_REDIRECT'
    except Exception as e:
        log['err'] = str(e)[:200]
    return log

results = []
print('=== US x3 (detailed) ===', flush=True)
for i in range(3):
    r = diag_once(f'US-{i+1}', 'US', 'US')
    results.append(r)
    print(json.dumps(r, ensure_ascii=False), flush=True)
    time.sleep(1)

print('\n=== US->BR x3 (detailed) ===', flush=True)
for i in range(3):
    r = diag_once(f'USBR-{i+1}', 'US', 'BR')
    results.append(r)
    print(json.dumps(r, ensure_ascii=False), flush=True)
    time.sleep(1)

with open('scripts/ba_diag_detailed_results.json', 'w') as f:
