...
print('=== US proxy + IE/EUR full diag ===', flush=True)
p = fetch_proxy('US')
s = make_session(p)
...
print(f'checkout: {r.status_code}', flush=True)
...
print(f'init: {ir.status_code} pmt={pmt} amt={amt}', flush=True)
...
print(f'tax: {tr.status_code} amt={amt}', flush=True)
...
print(f'pm: {pmr.status_code} id={pm}', flush=True)
...
if pmr_url:
    print(f'  redirect URL: {pmr_url[:80]}', flush=True)
    ...
    print(f'  final: {fin[:120]}', flush=True)
        ...
        for i in range(10):
            ...
            if pr.status_code == 200:
                ...
                if pmr_url:
                    rg = s.get(pmr_url, allow_redirects=True, timeout=20, headers={'Referer': 'https://pay.openai.com/'})
                    fin = str(getattr(rg, 'url', '')); bm = re.search(r'ba_token=(BA-[A-Za-z0-9]+)', fin)
                    print(f'  BA: {bm.group(1) if bm else "NONE"} url={fin[:120]}', flush=True)
                    break
                if pst == 'failed':
                ...
                print(f'  poll{i}: http={pr.status_code}', flush=True)
print('\n=== DONE ===', flush=True)
