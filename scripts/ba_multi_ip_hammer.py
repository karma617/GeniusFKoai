...
# 默认 US 全程 + US 账单；也支持 EUR 国家（tax_region 不传空 state）
CHECKOUT_COUNTRY = os.environ.get('BA_CHECKOUT_COUNTRY', 'US')
CHECKOUT_CURRENCY = os.environ.get('BA_CHECKOUT_CURRENCY', 'USD')
PROXY_REGION = os.environ.get('BA_PROXY_REGION', 'US')
MAX_ATTEMPTS = int(os.environ.get('BA_MAX_ATTEMPTS', '15'))
...
def try_once(attempt_no, country, currency, proxy_region):
    res = {
        'attempt': attempt_no,
    ...
    try:
        proxy = fetch_proxy(proxy_region)
        ...
        res['amt'] = amt
        if not (isinstance(pmt, list) and 'paypal' in [str(x).lower() for x in pmt]):
            res['err'] = 'no paypal pmt=' + str(pmt)
            return res

        # tax_region: 空 state 不传
        tb = {
            'eid': str(uuid.uuid4()),
...
def main():
    print(f'=== multi-IP hammer: proxy={PROXY_REGION} checkout={CHECKOUT_COUNTRY}/{CHECKOUT_CURRENCY} max={MAX_ATTEMPTS} stop_on_first={STOP_ON_FIRST} ===', flush=True)
    results = []
    for i in range(1, MAX_ATTEMPTS + 1):
        r = try_once(i, CHECKOUT_COUNTRY, CHECKOUT_CURRENCY, PROXY_REGION)
        results.append(r)
        print(
            f'  [{i:02d}] approve={str(r.get("approve_result","")):16s} ba_ok={str(r.get("ba_ok")):5s} amt={str(r.get("amt","")):6s} ba={r.get("ba","")} err={str(r.get("err",""))[:50]}',
            flush=True,
        )
    ...
    print(f'  overall_ba_ok={ba_ok}/{len(results)}', flush=True)

    out = 'scripts/ba_multi_ip_hammer_results.json'
