# -*- coding: utf-8 -*-
"""Dual-proxy PP BA extractor based on oaipay mode2/mode6 contract.
...
PROMO_PROXY = os.environ.get("BA_PROMO_PROXY", "TR").upper()
...
PROMO_WHEN = os.environ.get("BA_PROMO_WHEN", "after_init").lower()  # after_init|after_tax|none
...
OUT_JSON = os.environ.get("BA_OUT_JSON", "scripts/pp_dual_proxy_results.json")
...
def try_once(attempt_no: int) -> dict:
    ...
    try:
        ...
        ir = s_chk.post(f"{STRIPE_API_BASE}/payment_pages/{cs}/init", data=ib, headers=sh, timeout=30)
        ...
        if UI_MODE == "custom":
            ...
            tr = payment_pages_update(s_chk, cs, pk, bill_addr, ck)
            ...
            for i in range(8):
                time.sleep(1.5)
                pr = s_chk.get(f"{STRIPE_API_BASE}/payment_pages/{cs}", params=pp, headers=ph, timeout=8)
                if pr.status_code != 200:
                    continue
                pd = pr.json()
        ...
        if not redir:
            ...
            for i in range(8):
                time.sleep(1.2)
                pr = s_chk.get(f"{STRIPE_API_BASE}/payment_pages/{cs}", params=pp, headers={"Accept": "application/json", "Origin": "https://pay.openai.com", "Referer": "https://pay.openai.com/"}, timeout=8)
                if pr.status_code == 200:
