...
def test_parse_proxy_pool_and_region():
    ...
    assert mod.pick_proxy_from_pool(raw, attempt=1) == lines[0]
    assert mod.pick_proxy_from_pool(raw, attempt=2) == lines[1]
    assert mod.pick_proxy_from_pool(raw, attempt=3) == lines[0]



def test_find_redirect_scans_nested_paypal_urls():
    payload = {
        "id": "ppage_demo",
        "object": "checkout.session",
        "actions": [
            {"type": "noop", "url": "https://example.com/ignore"},
            {"type": "paypal", "url": "https://www.paypal.com/agreements/approve?ba_token=BA-NESTEDTOKEN123456"},
        ],
    }

    assert mod._find_redirect(payload).startswith("https://www.paypal.com/agreements/approve")


def test_payload_redirect_diagnostics_summarizes_relevant_fields():
    payload = {
        "id": "ppage_demo",
        "object": "checkout.session",
        "next_action": {"redirect_to_url": {"url": ""}},
        "return_url": "https://pay.openai.com/c/pay/cs_test/confirm",
    }

    diag = mod._payload_redirect_diagnostics(payload)

    assert "id=ppage_demo object=checkout.session" in diag
    assert "return_url" in diag
...
def test_extract_once_create_with_promo_skips_later_promotion_update():
    class FakeResp:
        def __init__(self, status_code=200, payload=None, text="", url=""):
            self.status_code = status_code
            self._payload = payload or {}
    ...
    bill_session.post.side_effect = [
        FakeResp(200, {
            "checkout_session_id": "cs_live_test456",
            "processor_entity": "openai_llc",
            "promo_campaign": {"status": "accepted"},
        }),
        FakeResp(200, {}),
    ...
    bill_session.get.side_effect = [
        FakeResp(200, {
            "submission_attempt": {
                ...
                "next_action": {
                    "type": "redirect_to_url",
                    "redirect_to_url": {"url": "https://pm-redirects.stripe.com/authorize/acct/sa2"},
                },
            }
        }),
        FakeResp(200, text="ok", url="https://www.paypal.com/agreements/approve?ba_token=BA-CREATEPROMO123456"),
    ]
    init_payload = {
        "init_checksum": "ck_1",
        "config_id": "cfg_1",
    ...
    with patch.object(mod, "build_protocol_session", side_effect=[bill_session, provider_session]), \
         patch.object(mod, "pick_proxy_from_pool", side_effect=["http://bill:1", "http://promo:1"]), \
         patch.object(mod.stripe_http, "stripe_init", return_value=init_payload), \
         patch.object(mod.stripe_http, "extract_expected_amount", return_value="0"), \
         patch.object(mod.stripe_http, "extract_confirm_expected_amounts", return_value=("0", "0")), \
         patch.object(mod.stripe_http, "extract_display_amounts", return_value={"subtotal": "0"}), \
         patch.object(mod.stripe_http, "build_confirm_return_url", return_value="https://pay.openai.com/c/pay/cs_live_test456/confirm"), \
         patch.object(mod.stripe_http, "build_confirm_referrer_url", return_value="https://pay.openai.com/c/pay/cs_live_test456"), \
         patch.object(mod.stripe_http, "StripeDeviceContext", return_value=MagicMock()), \
         patch.object(mod.stripe_http, "stripe_create_paypal_payment_method", return_value={"id": "pm_test_2"}), \
         patch.object(mod.stripe_http, "stripe_confirm_paypal_with_payment_method", return_value=confirm_payload), \
         patch.object(mod.stripe_http, "extract_paypal_redirect_url", side_effect=ValueError("no")), \
         patch.object(mod.proxy_pool, "report_success", return_value=None):
        result = mod.extract_ba_link(
            access_token="token",
            ...
            billing_proxy="gate-gb.kookeey.info:1000:u:p-GB-1-5m",
            promo_proxy="gate-jp.kookeey.info:1000:u:p-JP-2-5m",
            promo_create_mode="create_with_promo",
            max_attempts=1,
        )
    ...
    provider_session.post.assert_not_called()
    assert result["data"]["steps"]["promo_create_mode"] == "create_with_promo"
    assert result["data"]["steps"]["chatgpt_promo_http"] == "create_with_promo"


def test_extract_once_breaks_static_poll_without_redirect():
    class FakeResp:
        def __init__(self, status_code=200, payload=None, text="", url=""):
            self.status_code = status_code
            self._payload = payload or {}
    ...
    bill_session.post.side_effect = [
        FakeResp(200, {
            "checkout_session_id": "cs_live_static",
            "processor_entity": "openai_llc",
        }),
        FakeResp(200, {}),
        FakeResp(200, {"result": "approved"}),
    ...
    bill_session.get.side_effect = [
        FakeResp(200, {"id": "ppage_static", "object": "checkout.session"}),
        FakeResp(200, {"id": "ppage_static", "object": "checkout.session"}),
        FakeResp(200, {"id": "ppage_static", "object": "checkout.session"}),
    ]
    provider_session.post.side_effect = [FakeResp(200, {"promo_id": "plus-1-month-free"})]
    init_payload = {
        "init_checksum": "ck_1",
        "config_id": "cfg_1",
        "payment_method_types": ["card", "paypal"],
    ...
    with patch.object(mod, "build_protocol_session", side_effect=[bill_session, provider_session]), \
         ...
         patch.object(mod.stripe_http, "stripe_init", side_effect=[init_payload, synced_payload]), \
         patch.object(mod.stripe_http, "extract_expected_amount", side_effect=["1667", "0"]), \
         patch.object(mod.stripe_http, "extract_confirm_expected_amounts", return_value=("0", "0")), \
         patch.object(mod.stripe_http, "extract_display_amounts", return_value={"subtotal": "0"}), \
         patch.object(mod.stripe_http, "build_confirm_return_url", return_value="https://pay.openai.com/c/pay/cs_live_static/confirm"), \
         patch.object(mod.stripe_http, "build_confirm_referrer_url", return_value="https://pay.openai.com/c/pay/cs_live_static"), \
         patch.object(mod.stripe_http, "StripeDeviceContext", return_value=MagicMock()), \
         patch.object(mod.stripe_http, "stripe_create_paypal_payment_method", return_value={"id": "pm_test_static"}), \
         patch.object(mod.stripe_http, "stripe_confirm_paypal_with_payment_method", return_value={"submission_attempt": {"state": "requires_approval"}}), \
         patch.object(mod.stripe_http, "extract_paypal_redirect_url", side_effect=ValueError("no")):
        result = mod.extract_ba_link(
            access_token="token",
            email="demo@example.com",
    ...
    assert result["ok"] is False
    assert bill_session.get.call_count == mod.MAX_STATIC_NO_REDIRECT_POLLS
    assert result["data"]["steps"]["poll_static_no_redirect"] == mod.MAX_STATIC_NO_REDIRECT_POLLS
    assert any("切换下一代理" in str(e.get("desc")) for e in events)
